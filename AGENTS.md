...
reads `CLAUDE.md`, which imports this file - so there is one text, not two.
...
file has a declared writer** — named alike in the table and in that skill's own
`Writes:` line, **that every skill states
